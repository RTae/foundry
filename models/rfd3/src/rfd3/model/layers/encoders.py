...
class TokenInitializer(nn.Module):
    ...
    def forward(self, f):
        ...
        def init_tokens():
            with trace_range("RFD3/TokenInitializer/InputFeatureProcessing"):
            ...
            with trace_range("RFD3/TokenInitializer/PairFeatureInit"):
                # Embed Z_II
            ...
            with trace_range("RFD3/TokenInitializer/ReferencePositionEmbedding"):
            ...
            for i, block in enumerate(self.transformer_stack):
                with trace_range(f"RFD3/TokenInitializer/Pairformer/Block_{i}"):
                    S_I, Z_init_II = block(
                        S_I,
                        Z_init_II,
                        trace_prefix=f"RFD3/TokenInitializer/Pairformer/Block_{i}",
                    )

            with trace_range("RFD3/TokenInitializer/PairFeatureMix"):
        ...
        def init_atoms(S_init_I, Z_init_II):
            with trace_range("RFD3/TokenInitializer/OneDFeatureEmbedding"):
            ...
            if self.use_chunked_pll:
                ...
                if exists(self.atom_transformer):
                    with trace_range("RFD3/TokenInitializer/AtomTransformer"):
...
class DiffusionTokenEncoder(nn.Module):
    ...
    def forward(self, f, R_L, S_init_I, Z_init_II, C_L, P_LL, **kwargs):
        ...
        def token_embed(S_init_I, Z_init_II):
            with trace_range("RFD3/DiffusionTokenEncoder/SingleTransition"):
                S_I = S_init_I
                for b in range(2):
                    S_I = S_I + self.transition_1[b](S_I)

            with trace_range("RFD3/DiffusionTokenEncoder/DistogramProcessing"):
            ...
            with trace_range("RFD3/DiffusionTokenEncoder/PairFeatureProjection"):
            ...
            # Pairformer to mix
            for i, block in enumerate(self.pairformer_stack):
                with trace_range(f"RFD3/DiffusionTokenEncoder/Pairformer/Block_{i}"):
                    S_I, Z_II = block(
                        S_I,
                        Z_II,
                        trace_prefix=f"RFD3/DiffusionTokenEncoder/Pairformer/Block_{i}",
                    )

            return S_I, Z_II

        with trace_range("RFD3/DiffusionTokenEncoder"):
