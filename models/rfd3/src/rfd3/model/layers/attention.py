...
class GatedCrossAttention(nn.Module):
    ...
    def forward(self, q, kv, attn_mask=None, pair_bias=None):
        ...
        with trace_range("RFD3/Layers/GatedCrossAttention/LayerNorm"):
            q = self.ln_q(q)
            kv = self.ln_kv(kv)

        with trace_range("RFD3/Layers/GatedCrossAttention/QKV_Projection"):
            q, k, v, g = self.to_q(q), self.to_k(kv), self.to_v(kv), self.to_g(q)

        if self.kq_norm:
            with trace_range("RFD3/Layers/GatedCrossAttention/KQ_Norm"):
                k = self.k_norm(k)
                q = self.q_norm(q)

        with trace_range("RFD3/Layers/GatedCrossAttention/ReshapeHeads"):
            q, k, v, g = map(
                lambda t: rearrange(t, "b t n (h c) -> b h t n c", h=self.n_head),
                (q, k, v, g),
            )  # [B, tok, n, heads, c] ->  [B, heads, tok, n, c]

        with trace_range("RFD3/Layers/GatedCrossAttention/AttentionScores"):
            attn = einsum("bhtqc,bhtkc->bhtqk", q, k) * self.scale

        if pair_bias is not None:
            with trace_range("RFD3/Layers/GatedCrossAttention/PairBias"):
        ...
        with trace_range("RFD3/Layers/GatedCrossAttention/Softmax"):
            attn = F.softmax(attn, dim=-1)
        with trace_range("RFD3/Layers/GatedCrossAttention/Context"):
            attn_out = einsum("bhtqk,bhtkd->bhtqd", attn, v)
            attn_out = attn_out * g

        with trace_range("RFD3/Layers/GatedCrossAttention/Output_Projection"):
...
class LocalAttentionPairBias(nn.Module):
    ...
    def forward(
        ...
        with trace_range("RFD3/Layers/LocalAttentionPairBias/BuildIndices"):
        ...
        def do_attention(Q_L, C_L, P_LL):
            with trace_range("RFD3/Layers/LocalAttentionPairBias/AdaptiveNorm"):
                if exists(C_L):
            ...
            if use_kernel and not use_sparse_pll:
                ...
                with trace_range("RFD3/Layers/LocalAttentionPairBias/cuEquivarianceKernel"):
                    q, k, v, g, b = (
                        self.to_q(Q_L),
                        self.to_k(Q_L),
                        self.to_v(Q_L),
                ...
                with trace_range("RFD3/Layers/LocalAttentionPairBias/QKV_Projection"):
                    q, k, v, g = (
                        self.to_q(Q_L),
                        self.to_k(Q_L),
                        self.to_v(Q_L),
                ...
                if use_sparse_pll:
                    ...
                    with trace_range("RFD3/Layers/LocalAttentionPairBias/SparseAttention"):
            ...
            with trace_range("RFD3/Layers/LocalAttentionPairBias/Output_Projection"):
...
def sparse_pairbias_attention(
    ...
    """
    with trace_range("RFD3/Layers/PairBiasAttention/Entry"):
        D, L, c = Q.shape
    ...
    with trace_range("RFD3/Layers/PairBiasAttention/GatherKV"):
    ...
    with trace_range("RFD3/Layers/PairBiasAttention/GatherBias"):
        if gather_bias:
    ...
    # Split into heads
    with trace_range("RFD3/Layers/PairBiasAttention/ReshapeHeads"):
    ...
    # Do attention
    with trace_range("RFD3/Layers/PairBiasAttention/AttentionComputation"):
        attn = torch.einsum("...ld,...lkd->...lk", Q, K_gathered)
        attn = attn / sqrt(c // H)  # scale
        attn = attn + B_gathered  # add bias
    with trace_range("RFD3/Layers/PairBiasAttention/AttentionSoftmax"):
        attn = torch.softmax(attn, dim=-1)  # softmax over keys [D, H, L, k]
    with trace_range("RFD3/Layers/PairBiasAttention/WeightedSum"):
    ...
    # Optional gating
    if G is not None:
        with trace_range("RFD3/Layers/PairBiasAttention/Gating"):
            G = G.reshape(D, L, H, c // H).permute(0, 2, 1, 3)
            attn_out = attn_out * G

    # Merge heads
    with trace_range("RFD3/Layers/PairBiasAttention/MergeHeads"):
...
def pairbias_attention_(Q, K, V, B, H, valid_mask=None, G=None):
    ...
    """
    with trace_range("RFD3/Layers/PairBiasAttention/Full/Entry"):
        D, L, c = Q.shape
        k = L

    # Split into heads
    with trace_range("RFD3/Layers/PairBiasAttention/Full/ReshapeHeads"):
        Q = Q.reshape(D, L, H, c // H)
    ...
    with trace_range("RFD3/Layers/PairBiasAttention/Full/Permute"):
        Q = Q.permute(0, 2, 1, 3)  # [D, H, L, c // H]
        K = K.permute(0, 2, 1, 3)
        V = V.permute(0, 2, 1, 3)
        B = B.permute(0, 3, 1, 2)
    ...
    # Do attention
    with trace_range("RFD3/Layers/PairBiasAttention/Full/AttentionComputation"):
    ...
    with trace_range("RFD3/Layers/PairBiasAttention/Full/Softmax"):
        attn = torch.softmax(attn, dim=-1)  # softmax over keys [D, H, L, k]
    with trace_range("RFD3/Layers/PairBiasAttention/Full/WeightedSum"):
        attn_out = torch.einsum("...ij,...jc->...ic", attn, V)

    # Optional gating
    if G is not None:
        with trace_range("RFD3/Layers/PairBiasAttention/Full/Gating"):
            G = G.reshape(D, L, H, c // H).permute(0, 2, 1, 3)
            attn_out = attn_out * G

    # Merge heads
    with trace_range("RFD3/Layers/PairBiasAttention/Full/MergeHeads"):
        attn_out = attn_out.permute(0, 2, 1, 3)
        attn_out = attn_out.reshape(D, L, c).contiguous()
...
def _fused_full_pairbias_attention(
    *,
    ...
    """
    with trace_range("RFD3/Layers/PairBiasAttention/cuEquivariance/Prepare"):
        B, L, c = Q_L.shape
        H = num_heads
    ...
    # q, k, v as (B, H, L, DH)
    with trace_range("RFD3/Layers/PairBiasAttention/cuEquivariance/ReshapeQKV"):
    ...
    with trace_range("RFD3/Layers/PairBiasAttention/cuEquivariance/KernelCall"):
        out, _proj_z = cueq_attention_pair_bias(
            s=s,
            q=q,
            k=k,
